...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='xavier', gpu_ids=[], use_tanh=True, classification=True):
    ...
    elif which_model_netG == 'siggraphlocal':
        netG = SIGGRAPHLocalGenerator(input_nc, output_nc, norm_layer=norm_layer, use_tanh=use_tanh, classification=classification)
    elif which_model_netG == 'siggraphglobal':
        netG = SIGGRAPHGlobalGenerator(input_nc, output_nc, norm_layer=norm_layer, use_tanh=use_tanh, classification=classification)
    else:
...
# Local network in the original paper

class SIGGRAPHLocalGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, use_tanh=True, classification=True):
        super(SIGGRAPHLocalGenerator, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.classification = classification
        use_bias = True
        ...
        # layers for local hint network
        # upsample2 for hypercolumns
        self.upsample2 = nn.Sequential(*[nn.Upsample(scale_factor=2, mode='nearest'), ])
        model3_local = [nn.Conv2d(256, 384, kernel_size=3, stride=1, padding=1, bias=use_bias), ]
        model4_local = [nn.Conv2d(512, 384, kernel_size=3, stride=1, padding=1, bias=use_bias), ]
        model5_local = [nn.Conv2d(512, 384, kernel_size=3, stride=1, padding=1, bias=use_bias), ]
        model6_local = [nn.Conv2d(512, 384, kernel_size=3, stride=1, padding=1, bias=use_bias), ]
        model7_local = [nn.Conv2d(512, 384, kernel_size=3, stride=1, padding=1, bias=use_bias), ]
        model8_local = [nn.Conv2d(256, 384, kernel_size=3, stride=1, padding=1, bias=use_bias), ]
        local_model_class = [nn.Conv2d(384, 529, kernel_size=1, padding=0, dilation=1, stride=1, bias=use_bias), ]
        self.local3 = nn.Sequential(*model3_local)
        self.local4 = nn.Sequential(*model4_local)
        self.local5 = nn.Sequential(*model5_local)
        self.local6 = nn.Sequential(*model6_local)
        self.local7 = nn.Sequential(*model7_local)
        self.local8 = nn.Sequential(*model8_local)
        self.local_model_class = nn.Sequential(*local_model_class)
    ...
    def forward(self, input_A, input_B, mask_B):
        ...
        # local network foward
        conv4_3up = self.upsample2(conv4_3)
        conv5_3up = self.upsample2(conv5_3)
        conv6_3up = self.upsample2(conv6_3)
        conv7_3up = self.upsample2(conv7_3)
        
        local3 = self.local3(conv3_3)
        local4 = self.local4(conv4_3up)
        local5 = self.local5(conv5_3up)
        local6 = self.local6(conv6_3up)
        local7 = self.local7(conv7_3up)
        local8 = self.local8(conv8_3)

        local_out = nn.ReLU(True)(local3 + local4 + local5 + local6 + local7 + local8)
        #out_class = self.local_model_class(local_out)
        #print("Conv3_3 size:", conv3_3.shape)
        #print("Conv4_3 size:", conv4_3.shape, conv4_3up.shape, local3.shape)
        #print("Conv5_3 size:", conv5_3.shape, conv5_3up.shape, local4.shape)
        #print("Conv6_3 size:", conv6_3.shape, conv6_3up.shape, local5.shape)
        #print("Conv7_3 size:", conv7_3.shape, conv7_3up.shape, local6.shape)
        #print("Conv8_3 size:", conv8_3.shape, local8.shape)
        ...
        if(self.classification):
            ...
            #out_class = self.model_class(conv8_3.detach())
            out_class = self.local_model_class(local_out.detach())
            conv9_up = self.model9up(conv8_3) + self.model2short9(conv2_2)
            conv9_3 = self.model9(conv9_up)
            conv10_up = self.model10up(conv9_3) + self.model1short10(conv1_2)
            conv10_2 = self.model10(conv10_up)
...
# Global network in the original paper
class SIGGRAPHGlobalGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, norm_layer=nn.BatchNorm2d, use_tanh=True, classification=True):
        super(SIGGRAPHGlobalGenerator, self).__init__()
        self.input_nc = input_nc
        self.output_nc = output_nc
        self.classification = classification
        use_bias = True
        ...
        # 1x1 conv blocks for global hint network
        global_model1 = [nn.Conv2d(532, 512, kernel_size=1, bias=use_bias), ]
        global_model1 += [nn.ReLU(True), ]
        global_model2 = [nn.Conv2d(512, 512, kernel_size=1, bias=use_bias), ]
        global_model2 += [nn.ReLU(True), ]
        global_model3 = [nn.Conv2d(512, 512, kernel_size=1, bias=use_bias), ]
        global_model3 += [nn.ReLU(True), ]
        global_model4 = [nn.Conv2d(512, 512, kernel_size=1, bias=use_bias), ]
        global_model4 += [nn.ReLU(True), ]
        self.global_model1 = nn.Sequential(*global_model1)
        self.global_model2 = nn.Sequential(*global_model2)
        self.global_model3 = nn.Sequential(*global_model3)
        self.global_model4 = nn.Sequential(*global_model4)
    ...
    def forward(self, input_A, input_B, mask_B):
        ...
        conv3_3 = self.model3(conv2_2[:, :, ::2, ::2])
        conv4_3 = self.model4(conv3_3[:, :, ::2, ::2])
        #fuse the output of the green part with conv4_3
        conv4_4 = conv4_3 + global4
        conv5_3 = self.model5(conv4_4)
        conv6_3 = self.model6(conv5_3)
        conv7_3 = self.model7(conv6_3)
        conv8_up = self.model8up(conv7_3) + self.model3short8(conv3_3)
        conv8_3 = self.model8(conv8_up)
